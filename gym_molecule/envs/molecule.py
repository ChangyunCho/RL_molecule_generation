...
import gym
from gym_molecule.envs.synthetic_accessibility_scorer import calculateScore
from gym_molecule.dataset.dataset_utils import mol_to_nx,nx_to_mol
...
class load_dataset:
    def __init__(self, path):
        self.df = pd.read_csv(path, header=None, names=['smiles'])
...
class MoleculeEnv(gym.Env):
    ...
    def init(self,data_type='zinc',logp_ratio=1, qed_ratio=1,sa_ratio=1,reward_step_total=1,is_normalize=0,reward_type='gan',reward_target=0.5,has_scaffold=False,has_feature=False,is_conditional=False,conditional='low',max_action=128,min_action=20,force_final=False):
        ...
        if self.has_feature:
            ...
            self.possible_hybridization = np.array([Chem.rdchem.HybridizationType.SP,
                                                    Chem.rdchem.HybridizationType.SP2,
                                                    Chem.rdchem.HybridizationType.SP3,
                                                    Chem.rdchem.HybridizationType.SP3D,
                                                    Chem.rdchem.HybridizationType.SP3D2], dtype=object)
        ...
        cwd = os.path.dirname(__file__)
        if data_type=='gdb':
            path = os.path.join(os.path.dirname(cwd), 'dataset', 'gdb13.rand1M.smi.gz')  # gdb 13
        elif data_type=='zinc':
            path = os.path.join(os.path.dirname(cwd), 'dataset', '250k_rndm_zinc_drugs_clean_sorted.smi')  # ZINC
    ...
    def step(self, action):
        ...
        if self.check_valency():
            if self.mol.GetNumAtoms()+self.mol.GetNumBonds()-self.mol_old.GetNumAtoms()-self.mol_old.GetNumBonds()>0:
                ...
                self.smiles_list.append(self.get_final_smiles())
            else:
        ...
        if terminate_condition or self.force_final:
            ...
            if not self.check_chemical_validity():
                ...
                try:
                    ...
                    reward_final += reward_qed + reward_logp + reward_sa
            ...
            if self.is_conditional:
                info['reward_valid'] = self.conditional[-1]
            else:
    ...
    def reset(self,smile=None):
        ...
        self.smiles_list= [self.get_final_smiles()]
        self.counter = 0
        ob = self.get_observation()
        return ob
    ...
    def get_observation(self):
        ...
        for a in mol.GetAtoms():
            ...
            if self.has_feature:
                ...
                float_array = np.concatenate([(atom_symbol == self.possible_atom_types),
                                              ([not a.IsInRing()]),
                                              ([a.IsInRingSize(3)]),
                                              ([a.IsInRingSize(4)]),
                                              ([a.IsInRingSize(5)]),
                                              ([a.IsInRingSize(6)]),
                                              ([a.IsInRing() and (not a.IsInRingSize(3)) and (not a.IsInRingSize(4)) and (not a.IsInRingSize(5)) and (not a.IsInRingSize(6))])]).astype(float)
            else:
        ...
        for i in range(batch_size):
            ...
            for node_id, node in enumerate(graph_sub.nodes()):
                if self.has_feature:
                    ...
                    float_array = np.concatenate([(graph.node[node]['symbol'] == self.possible_atom_types),
                                                  ([len(cycle_info)==0]),
                                                  ([3 in cycle_len_info]),
                                                  ([4 in cycle_len_info]),
                                                  ([5 in cycle_len_info]),
                                                  ...
                                                  ([len(cycle_info)!=0 and (not 3 in cycle_len_info)
                                                   ...
                                                   and (not 6 in cycle_len_info)])]).astype(float)
                else:
...
class GraphEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.graph.number_of_nodes() + self.graph.number_of_edges()-self.graph_old.number_of_nodes() -             self.graph_old.number_of_edges() > 0:
            reward_step = self.reward_step_total / self.max_node # successfully added node/edge
        else:
            reward_step = -self.reward_step_total / self.max_node # edge
            self.graph = self.graph_old # already exists
...
def steric_strain_filter(mol, cutoff=0.82, max_attempts_embed=20, max_num_iters=200):
    ...
    :param mol: rdkit mol object
    :param cutoff: kcal/mol per angle. If minimized energy is above this threshold, then molecule fails the steric strain filter.
    :param max_attempts_embed: number of attempts to generate initial 3d coordinates
    :param max_num_iters: number of iterations of forcefield minimization
    :return: True if molecule could be successfully minimized, and resulting energy is below cutoff, otherwise False.
    """
    # check for the trivial cases of a single atom or only 2 atoms, in which case there is no angle bend strain energy (as there are no angles!)
...
#     return np.mean(SA_scores), np.std(SA_scores), np.mean(logP_values), np.std(logP_values), np.mean(cycle_scores), np.std(cycle_scores)
